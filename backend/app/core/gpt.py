...
from app.core.prompt import get_prompt, get_prompt_with_api_data
...
class GPTMetricEvaluator:
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "precision_issues": metrics.get("precision_issues", [])
                }
    ...
    def evaluate_with_api_data(self, screenshot_path: str, keyword: str, api_data: list) -> Dict[str, any]:
        """GPT Vision API + API 데이터를 사용하여 검색 결과 스크린샷 분석 및 메트릭 계산"""
        print(f"[GPT] API 데이터와 함께 평가 시작: 키워드 = {keyword}, 스크린샷 경로 = {screenshot_path}")
        ...
        if not self.client:
            ...
            precision_issues = []
            if api_data:
                # API 데이터의 모든 상품을 이슈로 표시 (시뮬레이션)
                for product in api_data:
                    image_url = product.get("thumbnail", "N/A")
                    if image_url == "N/A":
                        print(f"[GPT] 시뮬레이션 데이터에서 image_url이 'N/A'로 설정됨 - 상품: {product.get('goodsName', 'Unknown')}")
                ...
                print(f"[GPT] API 데이터가 없어 기본 시뮬레이션 데이터에서 image_url을 'N/A'로 설정")
                precision_issues = [{
                    "goods_no": 1234567,
                    "goods_name": "시뮬레이션 상품",
                    "image_url": "N/A",
        ...
        try:
            ...
            print("[GPT] GPT Vision API 호출 시작 (API 데이터 포함)")
            # GPT Vision API 호출
            response = self.client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": get_prompt_with_api_data(keyword, api_data)
                            },
                            {
                                "type": "image_url",
                                "image_url": {
            ...
            try:
                ...
                # precision_issues에 이미지 URL 추가
                if "precision_issues" in metrics and api_data:
                        ...
                        if goods_no:
                            ...
                            for product in api_data:
                                if str(product.get("goodsNo", "")) == str(goods_no):
                                    image_url = product.get("thumbnail", "N/A")
                                    if image_url == "N/A":
                                        print(f"[GPT] 상품 {goods_no}({issue.get('goods_name', 'Unknown')})의 thumbnail이 없어 image_url을 'N/A'로 설정")
                                    issue["image_url"] = image_url
                                    break
                            else:
                                print(f"[GPT] API 데이터에서 상품 {goods_no}({issue.get('goods_name', 'Unknown')})를 찾을 수 없어 image_url을 'N/A'로 설정")
                                issue["image_url"] = "N/A"
